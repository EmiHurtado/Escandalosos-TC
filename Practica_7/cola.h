...
void imprimir(Cola q){
   if(!esnueva(q)){
      printf("%c",primero(q));
      imprimir(desformar(q));
   }
}

int compararCola(Cola q1, Cola q2){
   if(primero(q1) == primero(q2)){
      return compararCola(desformar(q1),desformar(q2));
   } else if(esnueva(q1) && esnueva(q2)){
      return 1;
   } else{
      return 0;
   }
