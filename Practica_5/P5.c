...
void contarIden(char *arreglo, char *identificador, struct stat sb, int pos){
	...
	for(i = pos; i<sb.st_size;i++){
		...
		if(lon == strlen(identificador)){ // Son iguales por su longitud
		    if(arreglo[i] == '	' || arreglo[i] == ' ' || arreglo[i] == '\n' || arreglo[i] == '=' || arreglo[i] == '>' || arreglo[i] == '<' || arreglo[i] == '*' || arreglo[i] == '+' || arreglo[i] == '-'|| arreglo[i] == '{'|| arreglo[i] == '['|| arreglo[i] == ']'|| arreglo[i] == ')'|| arreglo[i] == ';'|| arreglo[i] == ',') // revisamos el siguiente valor
		        cont++;
...
void contarRes(char *arreglo, char *res, struct stat sb, int pos){
	...
	if(cont > 1) printf("%s se repite %d veces\n",res,cont);	
}
